...
public class HeapAdaptablePriorityQueue<K,V> extends HeapPriorityQueue<K,V> {
	...
	public Entry<K,V> insert (K k, V v) throws InvalidKeyException, InvalidPositionException, BoundaryViolationException {
		checkKey(k);
	  ...
	  public Entry<K,V> remove(Entry<K,V> entry) throws InvalidEntryException, 
	  		EmptyTreeException, InvalidPositionException, BoundaryViolationException {
	  ...
	  public K replaceKey(Entry<K,V> entry, K k) 
	    throws InvalidEntryException, InvalidPositionException, BoundaryViolationException, 
	    				InvalidKeyException   {
	    checkKey(k);
	    LocationAwareEntry<K,V> ee = checkEntry(entry);
	   ...
	   * @throws InvalidPositionException */
	  protected void swap(Position<Entry<K,V>> u, Position<Entry<K,V>> v) throws InvalidPositionException {
	   ...
	   * @throws InvalidPositionException */
	  protected Position replaceEntry(Position v, LocationAwareEntry<K,V> e) throws InvalidPositionException {
	  ...
	  protected LocationAwareEntry<K,V> checkEntry(Entry<K,V> ent) 
		  ...
		  if(ent == null || !(ent instanceof LocationAwareEntry))
			  new InvalidEntryException("Invalid entry");
	  ...
	  protected static class LocationAwareEntry<K,V> 
		  ...
		  public LocationAwareEntry(K k, V v) { super(k, v); }
		  public LocationAwareEntry(K k, V v, Position pos) { super(k, v); loc = pos; }
		  protected Position location() { return loc; }
