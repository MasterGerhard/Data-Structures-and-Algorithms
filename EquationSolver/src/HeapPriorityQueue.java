...
public class HeapPriorityQueue<K,V> implements PriorityQueue<K,V> {
	protected ArrayListCompleteBinaryTree<Entry<K,V>> heap;	// underlying heap
	...
	public Entry<K,V> removeMin() throws EmptyPriorityQueueException, EmptyTreeException, 
								InvalidPositionException, BoundaryViolationException {
		if (isEmpty())
			throw new EmptyPriorityQueueException("Priority Queue is empty");
	...
	/** Determines whether a give key is valid. */
	...
	protected void upHeap(Position<Entry<K,V>> v) throws InvalidPositionException, BoundaryViolationException {
	...
	/** returns a sorted Entry[] */
	public MyEntry<K,V>[] PQsort() throws EmptyPriorityQueueException, EmptyTreeException, 
			InvalidPositionException, BoundaryViolationException {
		MyEntry<K,V>[] sorted = new MyEntry[size()];
		int i = 0;
		while(!isEmpty()){
			sorted[i] = (MyEntry<K, V>) this.removeMin();
			i++;
		}
		return sorted;
	}
